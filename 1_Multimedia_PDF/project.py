...
# Get HUGGINGFACEHUB API key from environment variable
...
def expand_query(query: str, llm) -> str:
    ...
    chain = prompt | llm
    response = chain.invoke({"query": query})  # Pass as dict
    text = response.content  # Extract actual model output
    expanded_terms = [term.strip() for term in text.split(',')]
...
def rag_pipeline(query, qa_chain, vectorstore, images, tables):
    ...
    ### -----QA chain will automatically pass the context by using retrieval----- ###
    response = qa_chain.invoke({"query": query}) 
    ### ---QA chain will not performs it's own retrieval; the LLM will use the context you provided--- ###
